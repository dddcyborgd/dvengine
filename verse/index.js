...
/*
 *   DVVerse.enter({ canvas, doc | spaceId | tokenURI, claim, net:{ url, name, avatar }, senses:true, xr:true, legacy:false, spaceBase:'./live/spaces/' })
 ...
 *         dv:focus (a click on an aivatar) → the camera swaps to ArcballControls around that agent; Escape / focus(null) swaps back
 *         the local participant near an aivatar → aivatar.reachToward(participant) every frame (the arcball arm, predicted locally)
...
(function (global) {
  ...
  function enter(opts) {
    ...
    verse.space = null; verse.doc = null; verse.conn = null; verse.participant = null; verse.agents = {}; verse.zones = {}; verse.pieces = {}; verse.remotes = {}; verse.zone = null; verse.focused = null; verse.theme = null; verse.mesh = null; verse.host = null; verse.xr = null;
    ...
    return resolveDoc(opts).then(function (doc) {
      ...
      return (NS.world ? NS.world.ensure(doc) : Promise.resolve()).then(function () { return space.load(doc, { lazy: opts.lazy !== false }); }).then(function (summary) {
        ...
        space.on('dv:focus', function (d) {
          if (d.focused) { if (verse.focused && verse.focused !== d.id && verse.agents[verse.focused]) verse.agents[verse.focused].focus(false); verse.focused = d.id; if (verse.camera) verse.camera.focus(d.object); }
          else if (verse.focused === d.id) { verse.focused = null; if (verse.camera) verse.camera.unfocus(); }
        ...
        // tick
        var reachV = space.THREE ? new space.THREE.Vector3() : null;
        space.add('verse-tick', { tick: function (dt, t) {
          ...
          if (verse.participant && verse.participant.root) {
            var pr = verse.participant.root.position;
            for (var id in verse.agents) { var ag = verse.agents[id]; if (!ag.root) continue; var d = ag.root.position.distanceTo(pr); if (d < 3.4) { if (reachV) reachV.set(pr.x, pr.y + 1.3, pr.z); ag.reachToward(reachV || { x: pr.x, y: pr.y + 1.3, z: pr.z }); } }
  ...
  function connectNet(verse, opts, doc) {
    ...
    verse.conn = conn;
    conn.on('welcome', function (w) { verse.rung = w.rung | 0; verse.rank = w.rank || verse.rank; verse.emit('welcome', w); });
  ...
  NS.enter = enter; NS.describeVoucher = describeVoucher; NS.resolveDoc = resolveDoc; NS.version = '0.1.0'; NS.spaceBase = NS.spaceBase || './live/spaces/';
