...
export class SpawnSelectionMesh {
  static create(bbox?: Box3) {
    const size = bbox ? bbox.getSize(new Vector3()) : defSize;

    const height = size.y;

    const radius = Math.max(size.x, size.z) / 1.6;

    const baseY = bbox ? bbox.min.y : 0;
    ...
    const haloGeo = new CylinderGeometry(radius, radius, height, 32, 1, true);
