...
import { extractClientCoordFromEvent } from './utils/extract-client-coord-from-event';
import { notSupportedInOrthographicCamera } from './utils/not-supported-in-orthographic-camera';
import { EventDispatcher, Listener } from './event-dispatcher';
...
export class CameraControls extends EventDispatcher {
	...
	constructor(
		...
		const onPointerDown = ( event: PointerEvent ) => {
			...
			if ( ! this._enabled || ! this._domElement || !this.enabledMouse ) return;
	...
	update( delta: number ): boolean {
		...
		if ( this.dollyToCursor ) {
			...
			if ( isPerspectiveCamera( this._camera ) && this._changedDolly !== 0 ) {
				...
				// console.log( dollyControlAmount)

				// target position may be moved beyond boundary.
				this._boundary.clampPoint( newTargetEnd, newTargetEnd );
				const targetEndDiff = _v3B.subVectors( newTargetEnd, this._targetEnd );

				// const v = new _THREE.Vector3().copy(newTargetEnd).normalize()
	...
	toJSON(): string {
		...
		return JSON.stringify( {
			...
			dollySpeedIn 		 : this.dollySpeedIn,
	...
	fromJSON( json: string, enableTransition: boolean = false ): void {
		...
		this.dollySpeedIn          = obj.dollySpeedIn;
	...
	protected _dollyInternal = ( delta: number, x: number, y : number ): void => {
		...
		let speed = Math.sign(delta) > 0 ? this.dollySpeed : this.dollySpeedIn
		
		
		const dollyScale =  Math.sign(delta) > 0 ? Math.pow( 0.95, - delta * speed ) : Math.pow( 0.95, - delta * speed );
